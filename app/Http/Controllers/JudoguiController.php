...
use App\Models\Judogui;

class JudoguiController extends Controller
{
    
    public function index(){    //GET api/coche
        return response()->json(Judogui::all(), 200);
    }
    ...
    public function store(Request $request){ //POST api/coche + datos JSON
        try{
             $objeto = Judogui::create($request->all());
    ...
    public function show($id){ //GET api/coche/id
        $judogui = Judogui::find($id);
        if($judogui !=null){
            $paquetes = $judogui->paquetes;
        }
        return response()->json($judogui, 200);
    }
    ...
    public function update(Request $request, $id){ //PUT api/coche/id
        try{
            $judogui = Judogui::find($id);
            $resultado = $judogui->update($request->all());
